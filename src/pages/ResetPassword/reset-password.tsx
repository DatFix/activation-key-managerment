...
const ResetPasswordForm: React.FC = () => {
  ...
  const handleKeyDown = (index: number, event: React.KeyboardEvent<HTMLInputElement>) => {
    if (event.key === 'Backspace' && index > 0 && otpRefs[index].current?.value === '') {
      otpRefs[index - 1].current?.focus();
    }
  };
  ...
  const validateForm = (): boolean => {
    ...
    if (newPassword.length < 8) {
      ...
      newErrors.newPassword = 'Mật khẩu phải chứa ít nhất một chữ hoa, một chữ thường, một số và một ký tự đặc biệt';
    }
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement> | React.MouseEvent<HTMLAnchorElement, MouseEvent>) => {
    ...
    try {
      const response = await fetch('https://software-authentication.vercel.app/auth/reset-password', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        setErrors({ general: data.error || 'Đã xảy ra lỗi khi cập nhật mật khẩu.' });
      ...
      <div className="login-box">
        ...
        <form id="reset-password-form" onSubmit={handleSubmit}>
          <div className="user-box">
            ...
            {errors.otp && <small className="error-message" style={{padding: "55px 0 0 0"}}>{errors.otp}</small>}
            </div>
          ...
          <div className="user-box">
            ...
            <span id="eye" style={{ cursor: 'pointer', padding: "20px 0 0 0" }} onClick={handlePasswordVisibility}>
              {passwordVisible ? <FaEyeSlash /> : <FaEye />}
            </span>
            {errors.newPassword && <small className="error-message">{errors.newPassword}</small>}
          </div>
          <div className="user-box">
            <input
              type={passwordVisible ? 'text' : 'password'}
          ...
          {errors.general && <small id="error-message" style={{ color: 'rgb(249, 43, 43)' }}>{errors.general}</small>}
