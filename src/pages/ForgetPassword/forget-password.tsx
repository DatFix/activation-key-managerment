...
const ForgetPasswordForm: React.FC = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch('https://software-authentication.vercel.app/auth/forgot-password', {
        method: 'POST',
    ...
    <>
      ...
      <div className="login-box">
        ...
        <form id="form-login" onSubmit={handleSubmit}>
          ...
          {errors.email && <small id="error-email" style={{ color: 'rgb(249, 43, 43)' }}>{errors.email}</small>}
          <div className="forget-password">
            <small onClick={() => handleNavigate('')}><b>Quay lại</b></small>
          </div>
          <Link to="#" id="register-button" onClick={(e) => {
            e.preventDefault();
